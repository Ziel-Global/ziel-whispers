...
export default function AttendanceAdminPage() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("attendance")
        .select(
          "*, users!attendance_user_id_fkey(full_name, department, email, is_oversight, shift_start, has_custom_shift)",
        )
  ...
  const { data: openSessions = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("attendance")
        .select(
          "*, users!attendance_user_id_fkey(full_name, department, is_oversight)",
        )
        .is("clock_out", null)
  ...
  const { data: activeUsers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("users")
        .select(
          "id, full_name, department, is_oversight, shift_start, has_custom_shift",
        )
        .eq("status", "active")
  ...
  const filtered = useMemo(() => {
    ...
    return records
      .filter((r: any) => {
        if (
          workModeFilter !== "all" &&
          (r.work_mode || "").toLowerCase() !== workModeFilter
        )
          return false;
        ...
        if (searchQuery.trim()) {
          const q = searchQuery.trim().toLowerCase();
          if (!(r.users?.full_name || "").toLowerCase().includes(q))
            return false;
        }
        return true;
      })
  ...
  const lateCount = useMemo(
    () => records.filter((r: any) => r.clock_in && r.is_late).length,
    [records],
  );
  ...
  const formatDuration = (clockIn: string, clockOut: string | null) => {
    ...
    const secs = Math.max(
      0,
      Math.floor((end.getTime() - new Date(clockIn).getTime()) / 1000),
    );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const clockIn = editClockIn
        ? `${dateStr}T${editClockIn}:00+05:00`
        : editRecord.clock_in;
      const clockOut = editClockOut
        ? `${dateStr}T${editClockOut}:00+05:00`
        : null;
      ...
      if (clockIn && clockIn !== editRecord.clock_in) {
        ...
        const [userRes, settingsRes] = await Promise.all([
          supabase
            .from("users")
            .select("shift_start, has_custom_shift, working_days")
            .eq("id", editRecord.user_id)
            .single(),
          supabase.from("system_settings").select("key, value"),
        ]);
        const settings = settingsRes.data || [];
        const graceMinutes = Number(
          settings.find((s: any) => s.key === "late_grace_minutes")?.value ||
            15,
        );
        const defaultShiftStart =
          settings.find((s: any) => s.key === "default_shift_start")?.value ||
          "09:00";
        const workingDays = Number((userRes.data as any)?.working_days || 5);

        if (!(dayOfWeek === 0 || (dayOfWeek === 6 && workingDays === 5))) {
          const shiftStart =
            (userRes.data as any)?.has_custom_shift &&
  ...
  const exportCSV = () => {
    const header =
      "Employee,Department,Clock In,Clock Out,Duration,Work Mode,Late,Minutes Late,Notes\n";
    const rows = filtered
      .map((r: any) => {
  ...
  const filterItemClass =
    "focus:bg-[#FFEDD5] focus:text-[#09090B] cursor-pointer transition-colors";
    ...
    <div className="space-y-0">
        ...
        <Card className="p-4 border-yellow-200 bg-yellow-50/50 mb-6">
          ...
          <div className="divide-y divide-black/30">
            {staleOpenSessions.map((s: any) => (
              <p
                key={s.id}
                className="text-sm text-yellow-700 py-2 first:pt-0 last:pb-0"
              >
                <strong>{s.users?.full_name}</strong> — Open session since{" "}
                {formatPKTTime(s.clock_in)} on{" "}
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger
            className={cn("w-[130px] rounded-md", filterTriggerClass)}
          >
            <SelectValue placeholder="Status" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="present" className={filterItemClass}>
              Present
            </SelectItem>
            <SelectItem value="absent" className={filterItemClass}>
              Absent
            </SelectItem>
            <SelectItem value="late" className={filterItemClass}>
              Late
            </SelectItem>
          </SelectContent>
        </Select>
        ...
        <Select value={workModeFilter} onValueChange={setWorkModeFilter}>
          <SelectTrigger
            className={cn("w-[140px] rounded-md", filterTriggerClass)}
          >
          ...
          <SelectContent>
            ...
            <SelectItem value="remote" className={filterItemClass}>
              Remote
            </SelectItem>
            <SelectItem value="onsite" className={filterItemClass}>
              Onsite
            </SelectItem>
          </SelectContent>
        </Select>
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <div
              ...
              style={{
                gridTemplateColumns: "1.8fr 110px 110px 110px 110px 100px 80px",
              }}
            >
              <span>EMPLOYEE</span>
              <span>CLOCK IN</span>
              ...
              {filtered.map((r: any) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      gridTemplateColumns:
                        "1.8fr 110px 110px 110px 110px 100px 80px",
                    }}
                  >
                    {/* Employee */}
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                className="focus-visible:ring-[#EC6824]"
              />
              {editClockIn && (
                <p className="text-xs text-muted-foreground">
                  {to12Hour(editClockIn)}
                </p>
              )}
            </div>
            <div className="space-y-1">
