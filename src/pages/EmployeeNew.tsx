...
const staffSchema = z.object({
  ...
  join_date: z
    ...
    .refine((v) => new Date(v) <= new Date(), "Cannot be a future date"),
        ...
        <div className="space-y-2">
          ...
          {role && (
            <p className="text-xs text-muted-foreground pt-0.5">
              {role === "admin" &&
                "Full system access — manage users, settings & all data"}
              {role === "manager" &&
                "Manage team attendance, logs, leave, and projects"}
              {role === "employee" &&
                "Standard employee — submit logs, apply for leave"}
              {role === "client" &&
                "Client who owns the project — portal access to assigned projects"}
              {role === "client member" &&
                "Member of the client's team — portal access to assigned projects"}
              {role === "client portal" &&
                "Client portal user — portal access to assigned projects via /portal"}
            </p>
          )}
        </div>
...
function StaffForm({
  ...
  useEffect(() => {
    if (settings) {
      if (defaultShiftStart && !form.getValues("shift_start"))
        form.setValue("shift_start", defaultShiftStart);
      if (defaultShiftEnd && !form.getValues("shift_end"))
        form.setValue("shift_end", defaultShiftEnd);
      if (defaultReminder && !form.getValues("reminder_offset_minutes"))
        form.setValue("reminder_offset_minutes", defaultReminder);
    }
  ...
  const onSubmit = async (data: StaffFormData) => {
    if (!defaultShiftStart || !defaultShiftEnd || !defaultReminder) {
      toast.error(
        "System settings missing. Ask an admin to configure default shift times and reminder offset in Settings.",
      );
      return;
    }
    setSubmitting(true);
    try {
      const { data: result, error } = await supabase.functions.invoke(
        "invite-user",
        { body: { ...data, app_url: window.location.origin } },
      );
      if (error) {
      ...
      toast.success(
        `${selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)} created successfully`,
      );
      navigate("/employees");
    } catch (err: any) {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Fill in the details for the new {roleLabel.toLowerCase()} account
          </p>
        </div>
      </div>
                  ...
                  <FormItem>
                    ...
                    {field.value && (
                      <p className="text-xs text-muted-foreground">
                        Displayed as {formatTime12h(field.value)}
                      </p>
                    )}
                    <FormMessage />
                  </FormItem>
...
function ClientForm({ onBack }: { onBack: () => void }) {
  ...
  const onSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      const { data: result, error } = await supabase.functions.invoke(
        "invite-user",
        { body: { ...payload, app_url: window.location.origin } },
      );
      if (error) {
      ...
      toast.success(
        "Client created successfully. A welcome email with password setup link has been sent.",
      );
      navigate("/employees");
    } catch (err: any) {
      toast.error(err.message || "Unexpected error");
      ...
      <Card className="p-6">
        <div className="mb-6 p-4 rounded-lg bg-muted border border-border text-sm text-foreground">
          ...
          <p className="text-muted-foreground">
            The client will automatically receive a branded welcome email with a
            link to set their password and access the portal.
          </p>
        </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className="text-xs text-muted-foreground">
                    The client will be prompted to change this via the welcome
                    email link.
                  </p>
                  <FormMessage />
                </FormItem>
    ...
    try {
      ...
      if (error) {
        toast.error(
          error.message ||
            (portalMode
              ? "Failed to create client portal user"
              : "Failed to create client member"),
        );
        setSubmitting(false);
        return;
      }
      ...
      toast.success(
        portalMode
          ? "Client Portal user created successfully. A welcome email with password setup link has been sent."
          : "Client Member created successfully. A welcome email with password setup link has been sent.",
      );
      navigate("/employees");
    } catch (err: any) {
      toast.error(err.message || "Unexpected error");
    ...
    <div className="max-w-lg mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            {portalMode
              ? "Add New Client Portal User"
              : "Add New Client Member"}
          </h1>
          <p className="text-muted-foreground mt-1">
            {portalMode
              ? "Create an account for a client portal user"
              : "Create an account for a member of the client's team"}
          </p>
        </div>
      </div>
      ...
      <Card className="p-6">
        <div className="mb-6 p-4 rounded-lg bg-muted border border-border text-sm text-foreground">
          <p className="font-semibold mb-1">📧 Welcome Email</p>
          <p className="text-muted-foreground">
            The client member will automatically receive a branded welcome email
            with a link to set their password and access the portal.
          </p>
        </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className="text-xs text-muted-foreground">
                    The client member will be prompted to change this via the
                    welcome email link.
                  </p>
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Link to Client{" "}
                    {portalMode && <span className="text-destructive">*</span>}{" "}
                    {!portalMode && (
                  ...
                  <p className="text-xs text-muted-foreground">
                    {portalMode
                      ? "Required — this links the portal user to a client and auto-assigns their projects."
                      : "Linking to a client auto-syncs project access."}
                  </p>
                  <FormMessage />
                </FormItem>
              )}
