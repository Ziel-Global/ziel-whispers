...
export default function EmployeesPage() {
  ...
  const filterItemClasses =
    "focus:bg-[#FFEDD5] focus:text-[#09090B] data-[state=checked]:bg-[#FFF4EA] data-[state=checked]:text-[#09090B] [&[data-state=checked]_svg]:text-[#EC6824] cursor-pointer transition-colors";
        ...
        {isAdmin && (
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="icon"
              className="rounded-full border-[#E4E4E7] h-10 w-10 text-[#71717A]"
            >
            ...
            <Button
              ...
              className="rounded-md border-[#E4E4E7] text-[#09090B] font-semibold h-10 px-5 ml-2 shadow-sm"
            >
            ...
            <Button
              ...
              className="rounded-md bg-[#EC6824] hover:bg-[#EC6824]/90 text-white font-semibold h-10 px-6 shadow-sm"
            >
        ...
        <TabsContent value="list" className="mt-0">
          <div className="flex items-center gap-3 mb-6 flex-wrap">
            ...
            <Select value={deptFilter} onValueChange={setDeptFilter}>
              <SelectTrigger
                className={cn(
                  "w-[170px]",
                  getFilterTriggerClasses(deptFilter !== "all"),
                )}
              >
                <SelectValue placeholder="All Departments" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all" className={filterItemClasses}>
                  All Departments
                </SelectItem>
                {DEPARTMENTS.map((d) => (
                  <SelectItem key={d} value={d} className={filterItemClasses}>
                    {d}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger
                className={cn(
                  "w-[140px]",
                  getFilterTriggerClasses(statusFilter !== "all"),
                )}
              >
              ...
              <SelectContent>
                ...
                {STATUSES.map((s) => (
                  <SelectItem
                    key={s}
                    value={s}
                    className={cn(filterItemClasses, "capitalize")}
                  >
                    {s}
                  </SelectItem>
                ))}
            ...
            <Select value={typeFilter} onValueChange={setTypeFilter}>
              <SelectTrigger
                className={cn(
                  "w-[140px]",
                  getFilterTriggerClasses(typeFilter !== "all"),
                )}
              >
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger
                className={cn(
                  "w-[140px]",
                  getFilterTriggerClasses(roleFilter !== "all"),
                )}
              >
              ...
              <SelectContent>
                ...
                <SelectItem value="admin" className={filterItemClasses}>
                  Admin
                </SelectItem>
                <SelectItem value="manager" className={filterItemClasses}>
                  Manager
                </SelectItem>
                <SelectItem value="employee" className={filterItemClasses}>
                  Employee
                </SelectItem>
                <SelectItem value="client member" className={filterItemClasses}>
                  Client Member
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            {isLoading ? (
              ...
              <div>
                <div
                  ...
                  style={{
                    gridTemplateColumns: "1.5fr 1.5fr 1fr 100px 120px 80px",
                  }}
                >
                  <span>EMPLOYEE</span>
                  <span>EMAIL</span>
                  ...
                  {filtered.map((emp, index) => {
                    ...
                    return (
                      <div
                        key={emp.id}
                        className={cn(
                          "px-6 py-4 border-b border-[#F4F4F5] last:border-b-0 grid gap-4 items-center hover:bg-[#FFF1E6] transition-colors cursor-pointer",
                          rowBg,
                        )}
                        style={{
                          gridTemplateColumns:
                            "1.5fr 1.5fr 1fr 100px 120px 80px",
                        }}
                        onClick={() => navigate(`/employees/${emp.id}`)}
                      >
                        <div className="flex items-center gap-3">
                          <Avatar
                            className={`h-9 w-9 shrink-0 ${avatarColorClass}`}
                          >
                        ...
                        <div>
                          <Badge
                            className={cn(
                              "text-[11px] font-bold px-2.5 py-0.5 rounded-md border-transparent",
                              emp.status === "active"
                        ...
                        <div className="text-[13.5px] text-[#52525B] truncate">
                          {emp.join_date
                            ? format(new Date(emp.join_date), "MMM d, yyyy")
                            : "—"}
                        </div>
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {isAdmin && (
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 flex items-center justify-center rounded-lg bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEE2E2] transition-colors"
                              onClick={(e) => {
                                e.stopPropagation();
                                setDeletingUser(emp);
                              }}
          ...
          >
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  Delete user — {deletingUser?.full_name}
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <p className="text-sm text-destructive font-medium">
                  Warning: This will permanently delete the user account and all
                  related data (attendance, logs, leave requests, notifications,
                  balances, etc.). This action cannot be undone.
                </p>
                <p className="text-sm">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-lg">
                    Global Shift & Leave Settings
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    Used for employees who do not have a custom shift override.
                  </p>
                </div>
                <Button
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Currently: {formatTime12h(shiftStart)}
                      </p>
                    </div>
                    <div className="space-y-1.5">
                      ...
                      />
                      <p className="text-[11px] text-muted-foreground">
                        Currently: {formatTime12h(shiftEnd)}
                      </p>
                    </div>
                  </div>
                </div>
