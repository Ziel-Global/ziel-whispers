...
export default function MyLeavePage() {
  ...
  const { data: wfhRequests = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("*, users!remote_work_requests_reviewed_by_fkey(full_name)")
        .eq("user_id", user!.id)
  ...
  const handleWfhSubmit = async (e: React.FormEvent) => {
    ...
    if (wfhStartDate <= today) {
      toast.error("Start date must be a future date (tomorrow or later)");
      return;
    }
    ...
    const wfhDaysCount = countWorkingDays(
      wfhStartDate,
      wfhEndDate,
      workingDays,
    );
    ...
    const existing = wfhRequests.find(
      (r: any) =>
        r.start_date <= wfhEndDate &&
        r.end_date >= wfhStartDate &&
        (r.status === "pending" || r.status === "approved"),
    );
    if (existing) {
      toast.error(
        `You already have a ${existing.status} request that overlaps this date range.`,
      );
      return;
    }
  ...
  const maxLeaveHours = useMemo(
    () => calculateMaxLeaveHours(startDate),
    [startDate, today, shiftStart, shiftEnd],
  );
  ...
  const { data: annualEntitlement = 12 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("value")
        .eq("key", "annual_leave_entitlement")
        .maybeSingle();
  ...
  const { data: requests = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("leave_requests")
        .select(
          "*, leave_types(name), users!leave_requests_reviewed_by_fkey(full_name)",
        )
        .eq("user_id", user!.id)
        .order("created_at", { ascending: false });
      return data || [];
  ...
  const handleApply = async () => {
    if (
      !leaveCategory ||
      !startDate ||
      (leaveCategory !== "half_day" && !endDate)
    ) {
    ...
    const isNonWorkingDay = (d: number) =>
      d === 0 || (workingDays === 5 && d === 6);
    if (isNonWorkingDay(sDay) || isNonWorkingDay(eDay)) {
      toast.error(
        workingDays === 5
          ? "Date cannot be on Saturday or Sunday."
          : "Date cannot be on Sunday.",
      );
      return;
    }
    if (startDate < today) {
    ...
    if (leaveCategory === "half_day") {
      ...
      if (hoursNum > currentMaxHours) {
        toast.error(
          `You cannot take more hours than your remaining shift hours. You have ${currentMaxHours} hours remaining`,
        );
        return;
      }
    }
    ...
    if (isExhausted) {
      toast.error("You have exhausted your annual leave balance.");
      return;
    }
    if (finalDaysCount > remainingDays) {
      toast.error(
        `Insufficient balance. You have ${remainingDays} days remaining.`,
      );
      return;
    }
    ...
    const categoryLabel = LEAVE_CATEGORIES.find(
      (c) => c.value === leaveCategory,
    )?.label;
  ...
  const deleteRequest = async () => {
    ...
    if (error) {
      ...
      toast.error(
        "Could not delete leave request. It may have already been removed or you don't have permission.",
      );
    } else {
      await supabase.from("audit_logs").insert({
        actor_id: user!.id,
        ...
        <TabsContent value="leave" className="space-y-6 mt-0">
          ...
          <Card className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {currentLeaveYear.label}
            </p>
            <div className="flex items-baseline gap-2 mt-1">
              <span
                className={`text-2xl font-bold ${remainingDays <= 2 ? "text-destructive" : "text-foreground"}`}
              >
                {remainingDays}
              </span>
            ...
            {usedDays >= totalDays && (
              <p className="text-sm text-destructive mt-2 font-medium">
                Your leave limit for this leave year has been reached. Please
                contact the admin.
              </p>
            )}
          </Card>
          ...
          {filteredRequests.length === 0 ? (
            ...
            <div>
              ...
              {filteredRequests.map((r: any) => (
                <DataRow
                  key={r.id}
                  gridCols="1fr 112px 112px 80px 96px 112px 80px"
                >
                  <div>
                    <RowPrimary>
                      {r.hours
            ...
            <form onSubmit={handleWfhSubmit} className="space-y-4 max-w-lg">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (v <= today) {
                        toast.error(
                          "Start date must be a future date (tomorrow or later)",
                        );
                        return;
                      }
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (v <= today) {
                        toast.error(
                          "End date must be a future date (tomorrow or later)",
                        );
                        return;
                      }
                      const d = new Date(v + "T00:00:00");
              ...
              {wfhStartDate && wfhEndDate && (
                <p className="text-sm text-muted-foreground">
                  {countWorkingDays(wfhStartDate, wfhEndDate, workingDays)}{" "}
                  working day(s)
                </p>
              )}
              <div className="space-y-2">
                <Label htmlFor="wfhReason">
          ...
          {isExhausted ? (
            ...
            <div className="space-y-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (day === 0 || (day === 6 && workingDays === 5)) {
                          toast.error(
                            workingDays === 5
                              ? "Start date cannot be on Saturday or Sunday"
                              : "Start date cannot be on Sunday",
                          );
                          setStartDate("");
                          return;
                        }
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (day === 0 || (day === 6 && workingDays === 5)) {
                          toast.error(
                            workingDays === 5
                              ? "End date cannot be on Saturday or Sunday"
                              : "End date cannot be on Sunday",
                          );
                          setEndDate("");
                          return;
                        }
              ...
              {leaveCategory === "half_day" && (
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Remaining shift hours for the selected day: {maxLeaveHours}{" "}
                    hours
                  </p>
                </div>
              )}
              {leaveCategory === "half_day"
                ? startDate && (
                    <p className="text-sm text-muted-foreground">
                      Hourly Leave · {remainingDays} days remaining
                    </p>
                  )
                : workingDaysCount > 0 && (
                    <p className="text-sm text-muted-foreground">
                      {workingDaysCount} working day
                      {workingDaysCount > 1 ? "s" : ""} · {remainingDays} days
                      remaining
                    </p>
                  )}
              <div className="space-y-1">
