...
function UtilizationReport() {
  const [startDate, setStartDate] = useState(
    format(startOfMonth(new Date(getPKTDateString())), "yyyy-MM-dd"),
  );
  const [endDate, setEndDate] = useState(
    format(endOfMonth(new Date(getPKTDateString())), "yyyy-MM-dd"),
  );
  ...
  const { data: employees } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("users")
        .select(
          "id, full_name, department, shift_start, shift_end, created_at, working_days",
        )
        .eq("status", "active")
        .neq("role", "admin")
        .order("full_name");
  ...
  const rows = useMemo(() => {
    ...
    logs.forEach((l) => {
      if (l.user_id)
        loggedByUser[l.user_id] =
          (loggedByUser[l.user_id] || 0) + Number(l.hours);
    });

    return employees
      .map((e) => {
        const effectiveStart =
          e.created_at && e.created_at.split("T")[0] > startDate
        ...
        return {
          ...
          pct: Math.round(pct),
          status: pct < 70 ? "Low" : pct > 110 ? "Over" : "Good",
        };
      })
        ...
        <Card className="p-4">
          ...
          >
            <BarChart data={rows} layout="vertical" margin={{ left: 120 }}>
              <XAxis
                type="number"
                domain={[0, Math.max(120, ...rows.map((r) => r.pct))]}
                unit="%"
              />
              <YAxis
                type="category"
...
function HeatmapReport() {
  ...
  const grid = useMemo(() => {
    ...
    logs.forEach((l) => {
      ...
      const d = l.log_date;
      map[l.user_id][d] = (map[l.user_id][d] || 0) + Number(l.hours);
    });
  ...
  const cellColor = (h: number) => {
    if (h === 0) return "bg-muted";
    if (h <= 6) return "bg-green-200";
    if (h <= 9) return "bg-yellow-200";
    return "bg-red-200";
  };
        ...
        <div className="min-w-max">
          <div className="flex">
            ...
            {days.map((d) => (
              <div
                key={d.toISOString()}
                className={`w-7 text-center text-[10px] ${d.getDay() === 0 ? "text-muted-foreground/40" : "text-muted-foreground"}`}
              >
                {format(d, "d")}
              </div>
            ))}
          ...
          {grid.map((row) => (
            <div key={row.id} className="flex items-center">
              <div className="w-32 shrink-0 text-xs truncate pr-2">
                {row.name}
              </div>
              {days.map((d) => {
                ...
                const notJoined =
                  emp?.created_at && dateStr <= emp.created_at.split("T")[0];

                const cellWd = emp?.working_days ?? 5;
                const isCellWeekend =
                  d.getDay() === 0 || (d.getDay() === 6 && cellWd === 5);

                return (
                  <div
                    key={dateStr}
                    className={`w-7 h-6 m-px rounded-sm ${isCellWeekend ? "bg-muted/30" : notJoined ? "bg-gray-50/50" : cellColor(h)}`}
...
function MonthlySummaryReport() {
  ...
  const [month, setMonth] = useState(
    format(subDays(startOfMonth(new Date(getPKTDateString())), 1), "yyyy-MM"),
  );
  ...
  const { data: employees } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("users")
        .select("id, full_name, created_at, working_days")
        .eq("status", "active")
        .neq("role", "admin")
        .order("full_name");
      return data || [];
  ...
  const onsiteDays = Object.values(attendanceByDate).filter(
    (d) => d.onsite,
  ).length;
  const remoteDays = Object.values(attendanceByDate).filter(
    (d) => d.remote,
  ).length;

  const projectHours: Record<string, number> = {};
  logs?.forEach((l) => {
    const name = (l.projects as any)?.name || "No Project";
    projectHours[name] = (projectHours[name] || 0) + Number(l.hours);
  });

  const leaveByType: Record<string, number> = {};
  leave?.forEach((l) => {
    const name = (l.leave_types as any)?.name || "Unknown";
    leaveByType[name] = (leaveByType[name] || 0) + l.days_count;
  });
    ...
    <div className="space-y-4">
      ...
      {userId && logs && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Card className="p-4 text-center">
            <p className="text-sm text-muted-foreground">Late Logs</p>
            <p className="text-2xl font-bold">{lateLogs}</p>
          </Card>
          <Card className="p-4 text-center">
            <p className="text-sm text-muted-foreground">Onsite</p>
            <p className="text-2xl font-bold">{onsiteDays}</p>
          </Card>
          <Card className="p-4 text-center">
            <p className="text-sm text-muted-foreground">Remote</p>
            <p className="text-2xl font-bold">{remoteDays}</p>
          </Card>
        </div>
      )}
...
function AttendanceTrendReport() {
  ...
  const [startDate, setStartDate] = useState(
    format(subDays(new Date(getPKTDateString()), 30), "yyyy-MM-dd"),
  );
  ...
  const { data: attendance } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("attendance")
        .select("date, clock_in, work_mode, user_id")
        .gte("date", startDate)
        .lte("date", endDate);
      const { data } = await q;
      return data || [];
  ...
  const chartData = useMemo(() => {
    ...
    return days.map((d) => {
      ...
      dayAtt.forEach((a) => {
        if (
          a.clock_in &&
          (!firstClockInsByUser[a.user_id!] ||
            new Date(a.clock_in) < new Date(firstClockInsByUser[a.user_id!]))
        ) {
    ...
    <div className="space-y-4">
      <div className="flex gap-3 items-end flex-wrap">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => exportCSV(chartData, "attendance-trend.csv")}
        >
          <Download className="h-4 w-4 mr-1" />
          CSV
        </Button>
      ...
      <div ref={chartRef} className="space-y-4">
        <Card className="p-4">
          <h3 className="font-medium text-sm mb-2">Average Clock-in Time</h3>
        ...
        <Card className="p-4">
          <h3 className="font-medium text-sm mb-2">Daily Attendance Rate</h3>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <Card className="p-4">
          <h3 className="font-medium text-sm mb-2">Remote vs Onsite</h3>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {(logs || []).map((l) => (
              <TableRow key={l.id}>
                <TableCell>{format(parseISO(l.log_date), "MMM d")}</TableCell>
                <TableCell>{(l.users as any)?.full_name}</TableCell>
                <TableCell>{(l.projects as any)?.name || "—"}</TableCell>
                <TableCell>
...
function LeaveReport() {
  ...
  const { data: requests } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("leave_requests")
        .select("*, users(full_name), leave_types(name)")
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {(requests || []).map((r) => (
              <TableRow key={r.id}>
                <TableCell>{(r.users as any)?.full_name}</TableCell>
                <TableCell>{(r.leave_types as any)?.name}</TableCell>
                <TableCell>{format(parseISO(r.start_date), "MMM d")}</TableCell>
                <TableCell>{format(parseISO(r.end_date), "MMM d")}</TableCell>
...
function MissedLogsReport() {
  ...
  const { data: missed } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("missed_logs")
        .select("*, users(full_name, created_at, working_days)")
        .gte("log_date", startDate)
        .lte("log_date", endDate)
        .order("log_date", { ascending: false });
      return data || [];
  ...
  const filteredMissed = useMemo(() => {
    ...
    return missed.filter((m) => {
      ...
      if (dayLeave) {
        ...
        const loggedHoursToday = userLogsToday.reduce(
          (sum, l: any) => sum + Number(l.hours),
          0,
        );
          ...
          <TableBody>
            {filteredMissed.map((m) => (
              <TableRow key={m.id} className="bg-red-50/30">
                <TableCell>{(m.users as any)?.full_name}</TableCell>
                <TableCell>
                  {format(parseISO(m.log_date), "MMM d, yyyy")}
                </TableCell>
                <TableCell className="text-muted-foreground">
                  {formatPKTTime(m.detected_at)}
                </TableCell>
              </TableRow>
            ))}
