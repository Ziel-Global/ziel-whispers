...
describe("Section 1: Foundation (Sprint ↔ Status Derivation)", () => {
  ...
  it("Test 1.1: Sprint Status Auto-Derivation (link and unlink sprint)", async () => {
    ...
    const initialRows = await dbQuery<{
      status_id: string;
    ...
    await dbQuery(
      `UPDATE tasks SET sprint_id = '${sprintId}' WHERE id = '${task.id}';`,
    );
    ...
    const unlinkedRows = await dbQuery<{
      status_id: string;
      sprint_id: string | null;
    }>(`SELECT status_id, sprint_id FROM tasks WHERE id = '${task.id}';`);
    ...
    reporter.logResult({
      ...
      status: "PASS",
      notes:
        "Task linked and unlinked from sprint successfully adhering to workflow transition constraints",
    });
  }, 60000);
});
