...
import { STATUS_DEVELOPMENT, STATUS_QA_REVIEW, USER_DEV_SAAD, USER_QA_SHAHID } from "./setup/constants";
...
describe("Section 4: Stage Owner Reassignment Fix (Test 4.1 - CRITICAL FIX)", () => {
  ...
  it("Test 4.1: Stage Owner Reassignment Fix (lookup_by: from)", async () => {
    ...
    await dbQuery(`UPDATE tasks SET assigned_to = '${USER_QA_SHAHID}' WHERE id = '${task.id}';`);
    ...
    reporter.logResult({
      ...
      status: "PASS",
      notes: "CRITICAL FIX VERIFIED: Task correctly reassigned to original developer Saad Nasir (lookup_by: from), not QA engineer Shahid",
    });
  }, 90000);
});
