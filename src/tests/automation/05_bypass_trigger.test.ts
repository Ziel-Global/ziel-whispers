...
describe("Section 5: Bypass-Proofing (AFTER UPDATE Trigger)", () => {
  ...
  it("Test 5.2: Sprint Editing (Add/Remove Tasks) fires automation", async () => {
    ...
    await dbQuery(`UPDATE tasks SET status_id = '${STATUS_QA_REVIEW}' WHERE id = '${taskRemove.id}';`);
    ...
    await dbQuery(`UPDATE tasks SET status_id = '${STATUS_QA_REVIEW}' WHERE id = '${taskAdd.id}';`);
    // Sprint remove: task moves back to Development (unlinks from sprint)
    await dbQuery(`UPDATE tasks SET status_id = '${STATUS_DEVELOPMENT}' WHERE id = '${taskRemove.id}';`);
    ...
    expect(parseInt(afterAdd[0]?.cnt ?? "0")).toBeGreaterThan(parseInt(beforeAdd[0]?.cnt ?? "0"));
    expect(parseInt(afterRemove[0]?.cnt ?? "0")).toBeGreaterThan(parseInt(beforeRemove[0]?.cnt ?? "0"));

    reporter.logResult({
      testId: "5.2",
      testName: "Sprint Editing (Add/Remove Tasks)",
      status: "PASS",
      notes: "Automation fired for both tasks when their status changed via simulated sprint edit",
    });
  }, 60000);
  ...
  it("Test 5.3: Sprint Deletion Bypass (tasks reverted to Development, automation fires)", async () => {
    ...
    expect(parseInt(after[0]?.cnt ?? "0")).toBeGreaterThan(parseInt(before[0]?.cnt ?? "0"));

    reporter.logResult({
      testId: "5.3",
      testName: "Sprint Deletion Bypass",
      status: "PASS",
      notes: "Automation fired when sprint deletion reset task status back to Development",
    });
  }, 60000);
  ...
  it("Test 5.5: Negative Test - Title/Priority Edit Does NOT Fire Automation", async () => {
    ...
    reporter.logResult({
      ...
      status: "PASS",
      notes: "Automation correctly skipped — status_id unchanged, trigger guard IS NOT DISTINCT FROM worked",
    });
  }, 60000);
});
