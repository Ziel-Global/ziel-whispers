...
export async function setupTestEnvironment(): Promise<TestContext> {
  ...
  const taskTitles = [
    ...
    {
      title: "Fix duplicate transaction charge bug",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Refactor payment gateway webhook retry",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Design transaction receipt PDF exporter",
      priority: "low",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Sanitize JWT token storage in Keychain",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Update API documentation for v2 routes",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Fix OAuth token expiration state crash",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Add push notification for failed reload",
      priority: "low",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "QA review for dark mode theme toggle",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Fix login redirect loop on slow network",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Optimize SQL query for transaction history",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Audit biometric SDK memory consumption",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Add fingerprint fallback for older Android",
      priority: "low",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Fix currency symbol formatting in UI",
      priority: "low",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Validate session timeout modal popup",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Integrate Apple Pay express checkout",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Patch SSL certificate pinning validator",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Setup transaction summary email alert",
      priority: "medium",
      status: STATUS_DEVELOPMENT,
    },
    {
      title: "Enforce RBAC policy on refund endpoints",
      priority: "high",
      status: STATUS_DEVELOPMENT,
    },
  ];
  ...
  const taskValues = taskTitles
    .map(
      (t) => `('${projectId}', '${t.title}', '${t.status}', '${t.priority}')`,
    )
    .join(", ");

  const insertedTasks = await dbQuery<{
    id: string;
    title: string;
    status_id: string;
    priority: string;
  ...
  const rulesToInsert = [
    {
      ...
      actions: JSON.stringify([
        { type: "change_status", params: { status_id: STATUS_QA_REVIEW } },
      ]),
      status: "enabled",
      priority: 10,
    },
    {
      ...
      actions: JSON.stringify([
        { type: "assign_user", params: { user_id: USER_PM_SAMI } },
      ]),
      status: "enabled",
      priority: 10,
    },
      ...
      actions: JSON.stringify([
        {
          type: "add_comment",
          params: {
            body: "System Alert: A blocker was reported on this task.",
          },
        },
      ]),
      status: "enabled",
    ...
    },
    {
      ...
      actions: JSON.stringify([
        ...
        {
          type: "add_comment",
          params: { body: "Escalated to lead developer Saad Nasir." },
        },
      ]),
      status: "enabled",
      priority: 10,
    },
    {
      ...
      conditions: JSON.stringify([]),
      actions: JSON.stringify([
        { type: "change_status", params: { status_id: STATUS_DEVELOPMENT } },
      ]),
      status: "enabled",
      priority: 10,
    },
    {
      ...
      actions: JSON.stringify([
        { type: "change_status", params: { status_id: STATUS_DONE } },
      ]),
      status: "enabled",
      priority: 2,
    },
    {
      ...
      conditions: JSON.stringify([
        { field: "status_id", operator: "eq", value: STATUS_QA_REVIEW },
      ]),
      actions: JSON.stringify([
        { type: "assign_user", params: { user_id: USER_QA_SHAHID } },
      ]),
      status: "enabled",
      priority: 5,
    },
      ...
      actions: JSON.stringify([
        {
          ...
          params: {
            ...
            message_template:
              "Task {task_title} moved to QA in project {project_name}",
          },
        },
      ]),
      status: "enabled",
