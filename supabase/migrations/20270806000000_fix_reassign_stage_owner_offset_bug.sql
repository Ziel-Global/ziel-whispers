-- =======================================================================
-- Migration: Fix OFFSET 1 bug in run_automation_rules() for
--            reassign_to_stage_owner lookup.
--
-- Root cause: Both trg_record_task_status_history and
--             trg_run_automation_on_status_change are AFTER UPDATE triggers.
--             PostgreSQL fires AFTER triggers alphabetically, so
--             trg_record_task_status_history fires BEFORE
--             trg_run_automation_on_status_change.
--
--             The original migration used OFFSET 1 intending to "skip the
--             current transition row", but by the time automation runs the
--             history row for the CURRENT transition has already been
--             written. There is no duplicate row to skip.
--
--             When looking for from_status_id = 'Development' after a
--             QA->Dev rejection, the only matching row IS the Dev->QA
--             transition (what we want). OFFSET 1 skipped it, leaving
--             v_previous_owner = NULL and the task staying assigned to QA.
--
-- Fix: Remove OFFSET 1 from both the 'from' and 'to' branches.
-- =======================================================================
            ...
            IF v_task_id IS NOT NULL THEN
              IF v_target_status_id IS NOT NULL THEN
                IF v_lookup_by = 'from' THEN
                  -- "from" mode: find the assignee who last SENT the task AWAY FROM
                  -- the target status. History is already written by the time automation
                  -- runs (alphabetical trigger order), so LIMIT 1 returns the right row.
                  ...
                  LIMIT 1;
                ELSE
                  -- "to" mode (default): find whoever last ARRIVED AT the target status.
                  SELECT tsh.assigned_to_at_change INTO v_previous_owner
                  FROM public.task_status_history tsh
                  WHERE tsh.task_id = v_task_id
