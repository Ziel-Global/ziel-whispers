-- =======================================================================
-- Migration: 20270814000000_b3_reassignment_and_history_fix.sql
-- Description: B3 Reassignment Engine & Status History Enhancements
--   1. Update record_task_status_history() trigger function to populate
--      assigned_to_at_change at the exact moment of status transition.
--   2. Update run_automation_rules() reassign_to_stage_owner action to:
--      a) Verify previous owner is still an active project member.
--      b) Fall back to cross-project workload-balanced selection among
--         eligible project members holding the fallback role.
-- =======================================================================
...
BEGIN
  ...
  -- Read the session variable set by change_task_status() or default to 'system'
  BEGIN
  ...
  -- Capture assignee at the exact moment of transition:
  -- On UPDATE: OLD.assigned_to (the assigned user at the time the change occurred)
  -- On INSERT: NEW.assigned_to
...
-- Re-attach trigger to ensure it's up to date
