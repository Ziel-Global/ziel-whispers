-- =======================================================================
-- Migration: 20270815000000_b4_automation_chain_depth.sql
-- Description: B4 Automation Chain-Depth Protection & Loop Detection
--   1. Dynamically retrieves automation.max_chain_depth from project_settings
--      (project-level override -> global setting -> default 5).
--   2. Calculates current chain_depth for the root_event_id.
--   3. When chain_depth exceeds max_chain_depth:
--      a) Halts further automation execution (stops cascade).
--      b) Records an audit entry in automation_rule_runs with
--         result = 'failed' and error_message = 'Task Automation Loop Detected...'.
--      c) Emits a 'Task Automation Loop Detected' in-app notification to admins.
-- =======================================================================
...
BEGIN
  ...
  -- ── 1. Fetch dynamic max_chain_depth from project_settings_kv (B1 engine) ──
  BEGIN
    v_max_chain_depth := (public.get_project_setting(p_project_id, 'automation.max_chain_depth', '5'::jsonb))::text::INTEGER;
  EXCEPTION WHEN OTHERS THEN
    v_max_chain_depth := 5;
  END;

  v_max_chain_depth := COALESCE(v_max_chain_depth, 5);
  IF v_max_chain_depth < 1 THEN
    v_max_chain_depth := 5;
  END IF;

  -- ── 2. Calculate current chain depth for this root_event_id ─────────────
  ...
  -- ── 3. B4 Loop Detection Check ─────────────────────────────────────────────
  IF v_chain_depth > v_max_chain_depth THEN
    -- Get first matching enabled rule ID for audit log reference
    SELECT id INTO v_first_rule_id
    ...
    -- Record failed run entry in automation_rule_runs
    IF v_first_rule_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        v_actual_root_id,
        'failed',
        'Task Automation Loop Detected: Exceeded max chain depth ' || v_max_chain_depth || ' (current depth: ' || v_chain_depth || ')'
      );
    END IF;

    -- Emit Task Automation Loop Detected notification to admins/managers
    INSERT INTO notifications (user_id, type, channel, metadata, read)
    SELECT u.id, 'system', 'in_app',
      jsonb_build_object(
        'title', 'Task Automation Loop Detected',
        'message', 'Automation chain halted: Exceeded max depth limit of ' || v_max_chain_depth || ' for project.',
        'project_id', p_project_id,
        'chain_depth', v_chain_depth,
        'max_chain_depth', v_max_chain_depth
      ),
      false
    FROM users u
    WHERE u.role IN ('admin', 'manager');

    -- Halt further rule execution and cascading
    RETURN;
  END IF;
